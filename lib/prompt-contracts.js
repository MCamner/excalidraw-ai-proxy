...
// and label the flow. Diagram size is governed by the runtime node budget so
// prompt contracts do not compete with MERMAID_MAX_NODES.
const ARCHITECTURE_SYSTEM_PROMPT =
  "You generate valid Mermaid source for Excalidraw's Mermaid parser describing a software architecture. Return only Mermaid code, no markdown fences, no explanation. Use flowchart TD. Group related components with subgraph blocks that have descriptive titles. Show data and control flow with labeled edges. Use simple ASCII node IDs like A, B, S1, DB1. Always wrap every node label, subgraph title, and edge label in double quotes, for example S1[\"API Gateway\"], subgraph SVC[\"Services\"], and A -->|\"reads/writes\"| DB1. Do not use classDef, style lines, bare class statements with raw CSS such as class LEGEND fill:#fff, HTML, JSON, or prose. Do not use reserved words such as end as node IDs. Prefer a concise, readable architecture and avoid unnecessary intermediate nodes.";

// The architecture contract is materially stricter than the default one: it
// mandates subgraphs and fully quoted labels. Selecting it by mistake silently
// narrows what the user asked for, so the heuristic is deliberately two-tier.
// Both tiers are bilingual (sv/en) and match on stems so Swedish compounds and
// inflections are covered.
...
export function systemPromptFor(prompt, { mode } = {}) {
  return promptContracts[resolveContractMode(prompt, { mode })];
}

// The selected contract also decides which model the request is worth: an
// architecture diagram is the expensive case, an ordinary flowchart is not.
export function resolveContractMode(prompt, { mode } = {}) {
  const explicitMode = normalizeMode(mode);
  if (explicitMode) {
    return explicitMode;
  }

  return isArchitecturePrompt(prompt) ? "architecture" : "default";
}
