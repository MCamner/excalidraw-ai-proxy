...
export function registerExcalidrawRoutes(app, { openai, config }) {
  ...
  app.post("/v1/ai/diagram-to-code/generate", async (req, res) => {
    try {
      ...
      const selection = selectModel("diagram-to-code", config);
      const { model } = selection;
      res.locals.modelRoute = {
        task: "diagram-to-code",
        model,
  ...
  app.post("/v1/ai/text-to-diagram/chat-streaming", async (req, res) => {
    try {
      ...
      const task = contractMode === "architecture" ? "text-to-diagram:architecture" : "text-to-diagram";
      const selection = selectModel(task, config);
      const { model } = selection;
      res.locals.modelRoute = {
        task,
        model,
      ...
      if (normalization.autoRepairApplied) {
        const repairSelection = selectModel("mermaid-repair", config);
        res.locals.modelRoute.repairModel = repairSelection.model;
        res.locals.modelRoute.repairModelReason = repairSelection.reason;
      }
