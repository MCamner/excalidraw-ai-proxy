...
// Rank alone is too coarse inside one severity. A retry that takes a diagram
// from 15 nodes to 9 against a budget of 8 has done most of the work, but both
// candidates are soft, so ranking them equal discards the better one and
// reports the attempt as unresolved. Positive means `candidate` is better.
export function compareDiagnoses(candidate, incumbent) {
  const byRank = diagnosisRank(candidate) - diagnosisRank(incumbent);
  if (byRank !== 0) {
    return byRank;
  }

  if (
    candidate?.errorType === mermaidErrorTypes.nodeLimitExceeded &&
    incumbent?.errorType === mermaidErrorTypes.nodeLimitExceeded
  ) {
    return (incumbent.nodeCount || 0) - (candidate.nodeCount || 0);
  }
