...
const REPAIR_SYSTEM_PROMPT =
  "You repair Mermaid source so Excalidraw's Mermaid parser can import it. Apply the smallest fix that resolves the reported error and keep the diagram's meaning, nodes, and edges intact. Return only Mermaid source, no markdown fences and no explanation.";
...
const REPAIR_INSTRUCTIONS = {
  ...
  [mermaidErrorTypes.nodeLimitExceeded]:
    "The diagram has too many nodes to stay readable. Merge related nodes and group them in subgraphs until it fits the limit, without losing the main flow.",
};
...
export async function repairMermaid(rawMermaid, { openai, config = {}, prompt = "" } = {}) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    ...
    if (diagnosisRank(diagnosis) > diagnosisRank(best.diagnosis)) {
      best = next;
    ...
    current = diagnosisRank(next.diagnosis) >= diagnosisRank(current.diagnosis) ? next : current;
  }
...
export function buildRepairPrompt(diagnosis, mermaid, { prompt = "" } = {}) {
  const instruction =
    REPAIR_INSTRUCTIONS[diagnosis.errorType] || REPAIR_INSTRUCTIONS[mermaidErrorTypes.syntaxError];
...
async function requestRepair(current, { openai, config, prompt }) {
  ...
  const response = await openai.chat.completions.create({
    ...
    messages: [
      ...
      {
        role: "user",
        content: buildRepairPrompt(current.diagnosis, current.mermaid, { prompt }),
      },
    ],
  });
...
function repairOutcome(before, after) {
  ...
  if (diagnosisRank(after) > diagnosisRank(before)) {
    return "improved";
  }
  return "unresolved";
}
